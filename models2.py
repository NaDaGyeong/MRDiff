...
#from diffusion_module2_da import p_losses, q_sample, p_sample
#from diffusion_module2_da1 import *
device = 'cuda'
...
class ConditionalDiffusionTrainingNetworkwm(nn.Module):
	def __init__(self,nr_feats, window_size, batch_size, noise_steps, denoise_steps, timestep_list, lambdas, train=True):
		super().__init__()
		self.dim = min(nr_feats, 16)
		self.nr_feats = nr_feats
		self.window_size = window_size
	...
	def forward(self, x):
		diffusion_loss = None
		x_recon = None
		reconstruced_xs=None
		#timestep_lists=[30,50,70,110,130]
		...
		if self.training:
			...
			for t in self.timestep_list:
				x_recon = sample(self.denoise_fn, shape=(x.shape[0], 1, self.window_size, self.nr_feats), x_start=x, denoise_steps=t)
				reconstruced_xs.append(x_recon)
		return diffusion_loss, reconstruced_xs
