...
if __name__ == '__main__':
    ...
    if model:
        ...
        lossFinal = np.mean(np.array(loss0), axis=1)
        np.save(f'./dataset/{args.dataset}_DiffusionAE_scores_100_50.npy', lossFinal)
        np.save(f'./dataset/{args.dataset}_DiffusionAE_recons_100_50.npy', samples)
        ...
        np.save(f'./dataset/{args.dataset}_epoch100_100_50_scores.npy', lossFinal)
        np.save(f'./dataset/{args.dataset}_epoch100_100_50_recons.npy', samples)
        labelsFinal = (np.sum(labels, axis=1) >= 1) + 0
